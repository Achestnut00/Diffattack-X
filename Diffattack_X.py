...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        cross_entropy_loss = -(targets_one_hot * log_prob).sum(dim=-1)
        focal_loss = self.alpha * focal_weights * cross_entropy_loss
...
class GroupedSEAttention(nn.Module):
    def __init__(self, channel=512, reduction=4, groups=8):
        super().__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc1 = nn.Conv2d(channel, channel // reduction, kernel_size=1, groups=groups, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(channel // reduction, channel, kernel_size=1, groups=groups, bias=False)
...
class LabelSmoothingCrossEntropyLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # 平滑标签
        targets_one_hot = (1 - self.smoothing) * targets_one_hot + self.smoothing / inputs.size(-1)

        loss = -(targets_one_hot * log_prob).sum(dim=-1).mean()
        return loss
...
class LearnableWeightedMSELoss(nn.Module):
    def __init__(self):
        super(LearnableWeightedMSELoss, self).__init__()
        self.weight = None
        self.mse_loss = nn.MSELoss(reduction='none')  # 使用 'none' 以便逐元素计算损失

    def initialize_weights(self, input_tensor):
        # 获取输入张量的设备
        device = input_tensor.device
        # 初始化可学习的权重，其形状与输入损失的形状一致，并将其移动到相同的设备上
        self.weight = nn.Parameter(torch.ones_like(input_tensor, device=device))

    def forward(self, prediction, target):
        if self.weight is None:
            raise RuntimeError("Weight has not been initialized. Call 'initialize_weights' with an input tensor.")
        ...
        # 使用学习到的权重计算加权损失
        weighted_loss = self.weight * mse_loss
        # 对所有元素的损失求均值
...
class BiLevelRoutingAttention(nn.Module):
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        ### loss  begain
        # attn_weight = attn_weight.view(2, -1, attn_weight.shape[1], attn_weight.shape[2],
        #                                attn_weight.shape[3])
        #
        # # 获取基础注意力和替换注意力
        # # print("our")
        # # print(attn_weight.shape)
        # attn_base = attn_weight[0]  # 第一个批次的注意力权重
        # attn_replace = attn_weight[1:]  # 其余批次的注意力权重
        #
        # # 扩展基础注意力
        # replaced_attn = attn_base.unsqueeze(0).expand(attn_replace.shape[0], *attn_base.shape)
        #
        # # 计算损失
        # self.loss += self.criterion(attn_replace, replaced_attn)
        #
        # # 将注意力权重调整回原始形状
        # attn_weight = attn_weight.view(-1, attn_weight.shape[2], attn_weight.shape[3], attn_weight.shape[4])
        ### loss end
